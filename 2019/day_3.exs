defmodule Main do
  # X Parse each instruction
  # X Turn instruction (R7) into a stream of transform functions
  # X Step through each transform function stream, adding / constructing the set along the way
  # X Get set intersection
  # X Map to manhattan distances
  ...
  def parse_instruction(instruction) do
    ...
    Stream.map(1..distance, fn _ -> operation end)
  end

  def as_manhattan({x, y}) do
    abs(x) + abs(y)
  end

  def get_operation_stream(instructions) do
    Stream.flat_map(String.split(instructions, ",", trim: true), &parse_instruction/1)
  end

  def get_points(operation_stream) do
    operation_stream
    |> Enum.reduce([{0, 0}], fn op, [latest | rest] ->
      [op.(latest) | [latest | rest]]
    end)
    |> Enum.reverse()
  end

  def as_points_map_with_length(points) do
    points
    |> Stream.with_index()
    |> Enum.reduce(Map.new(), fn {point, length}, acc ->
      acc |> Map.put_new(point, length)
    end)
  end

  def as_points_set(point_length_map) when is_map(point_length_map) do
    point_length_map
    |> Map.keys()
    |> as_points_set
  end

  def as_points_set(points) do
    points
    |> MapSet.new()
  end

  def without_zero(points = %MapSet{}) do
    points
    |> MapSet.delete({0, 0})
  end

  def solve_manhattan(line1, line2) do
    points1 =
      line1
      |> get_operation_stream()
      |> get_points()
      |> as_points_set()
      |> without_zero()
    ...
    |> Stream.map(&as_manhattan/1)
    |> Enum.min()
  end

  def solve_length_optimization(line1, line2) do
    points_map_1 =
      line1
      |> get_operation_stream()
      |> get_points()
      |> as_points_map_with_length()
    ...
    intersecting_points =
      MapSet.intersection(
        points_map_1 |> as_points_set() |> without_zero(),
        points_map_2 |> as_points_set() |> without_zero()
      )

    intersecting_points
    |> Stream.map(fn point ->
      Map.fetch!(points_map_1, point) +
        Map.fetch!(points_map_2, point)
    end)
    |> Enum.min()
  end

  def main(line1, line2) do
    IO.puts("Manhattan distance: #{solve_manhattan(line1, line2)}")
    IO.puts("Optimized length: #{solve_length_optimization(line1, line2)}")
  end
end
